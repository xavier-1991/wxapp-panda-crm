    ...
    <view v-if="isShow"
        ...
        <view v-if="type!='time'"
            ...
            <view class="picker-modal-header">
                ...
                <view class="picker-icon picker-icon-zuo"
                      :hover-stay-time="100"
                      hover-class="picker-icon-active"
                      @click="onSetMonth('-1')"></view>
                <text class="picker-modal-header-title">{{title}}</text>
                <view class="picker-icon picker-icon-you"
                      :hover-stay-time="100"
                      hover-class="picker-icon-active"
                      @click="onSetMonth('+1')"></view>
                <view class="picker-icon picker-icon-youyou"
                      :hover-stay-time="100"
                      hover-class="picker-icon-active"
                      @click="onSetYear('+1')"></view>
            </view>
            <swiper class="picker-modal-body"
                    :circular="true"
                    :duration="200"
                    :skip-hidden-item-layout="true"
                    :current="calendarIndex"
                    @change="onSwiperChange">
                <swiper-item class="picker-calendar"
                    ...
                    <view class="picker-calendar-view"
                        ...
                        <!-- 信息样式 -->
                        <view v-show="date.tips"
                              class="picker-calendar-view-tips">{{date.tips}}</view>
                    </view>
                </swiper-item>
            </swiper>
        ...
        <view v-if="showTimePicker"
            ...
            <view class="picker-modal picker-time">
                ...
                <picker-view class="picker-modal-time"
                             indicator-class="picker-modal-time-item"
                             :value="timeValue"
                             @change="onTimeChange">
                    <picker-view-column>
                        <view v-for="(v,i) in 24"
                              :key="i">{{i<10?'0'+i:i}}时</view>
                    </picker-view-column>
                    <picker-view-column>
                        <view v-for="(v,i) in 60"
                              :key="i">{{i<10?'0'+i:i}}分</view>
                    </picker-view-column>
                    <picker-view-column v-if="showSeconds">
                        <view v-for="(v,i) in 60"
                              :key="i">{{i<10?'0'+i:i}}秒</view>
                    </picker-view-column>
                </picker-view>
                <view class="picker-modal-footer">
                    <view class="picker-modal-footer-info">
...
export default {
    ...
    methods: {
        ...
        setValue(value) {
            ...
            if (value) {
                ...
                if (this.checkeds.length)
                    this.date = new Date(this.checkeds[0]);
            } else {
                if (this.isContainTime) {
                    this.beginTime = [
                        this.date.getHours(),
        ...
        //选中日期
        onSelectDate(date) {
            if (~this.type.indexOf("range") && this.checkeds.length == 2)
                this.checkeds = [];
            else if (!~this.type.indexOf("range") && this.checkeds.length)
                this.checkeds = [];
            ...
            if (this.type == "time") {
                ...
                if (this.isMultiSelect) {
                    ...
                    if (this.isContainTime) {
                        ...
                        if (this.showSeconds)
                            newDate.setSeconds(this.beginTime[2]);
                    }
                    result.value = DateTools.format(
                        newDate,
                        this.format
                            ? this.format
                            : defaultFormat[
                                  this.isContainTime ? "datetime" : "date"
                              ]
                    );
                    result.date = newDate;
                }
    ...
    computed: {
        BeginTitle() {
            let value = "未选择";
            if (this.checkeds.length)
                value = DateTools.format(this.checkeds[0], "yy/mm/dd");
            return value;
        },
        EndTitle() {
            let value = "未选择";
            if (this.checkeds.length == 2)
                value = DateTools.format(this.checkeds[1], "yy/mm/dd");
            return value;
        },
        PickerTimeTitle() {
